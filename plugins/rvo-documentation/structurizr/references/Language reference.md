...
published:
created: 2026-07-15
description: "Documentation for the Structurizr tooling."
tags:
  - "clippings"
---
Table of contents

This page describes the language syntax and grammar, with angle brackets (`<...>`) used to show required properties, and square brackets (`[...]`) used to show optional properties. Most statements are of the form: `keyword <required properties> [optional properties]`
...
The `group` keyword provides a way to define a named grouping of elements, which will be rendered as a boundary around those elements. Groups can be nested; see [DSL Cookbook - Groups](https://docs.structurizr.com/dsl/cookbook/groups) for more information.
...
See [Notation](https://docs.structurizr.com/server/diagrams/notation) for a description of which groups are shown for a given diagram type.
...
See [DSL cookbook - Custom elements](https://docs.structurizr.com/dsl/cookbook/custom-elements) for an example.
...
- `--https->` is a relationship based upon an archetype - see [Relationship achetypes](https://docs.structurizr.com/dsl/archetypes#relationship-archetypes) for details.
...
`instances` is used to set the number of instances of a deployment node. This can either be a static number, or a range (e.g. 0..1, 1..3, 5..10, 0..N, 0..*, 1..N, 1..*, etc).
...
The `perspectives` block is used to define one or more named perspectives for an element or relationship. See [DSL cookbook - Perspectives (static)](https://docs.structurizr.com/dsl/cookbook/perspectives-static/) and [DSL cookbook - Perspectives (dynamic)](https://docs.structurizr.com/dsl/cookbook/perspectives-dynamic/) for how these are used.
...
The first usage scenario is to extend an existing element that has been defined via the DSL. See [DSL cookbook - Workspace extension](https://docs.structurizr.com/dsl/cookbook/workspace-extension/) for a usage example.
...
The `!elements` keyword can be used to find a set of elements via an [element expression](https://docs.structurizr.com/dsl/expressions#element-expressions) in order to perform bulk operations on them.
...
The `!relationships` keyword can be used to find a set of relationships via a [relationship expression](https://docs.structurizr.com/dsl/expressions#relationship-expressions) in order to perform bulk operations on them.
...
If a workspace doesn’t have a `views` block, or the `views` block doesn’t define any views, a [default set of views](#convention-over-configuration-useful-defaults) will be defined for you. Defining one or more views in the `views` block will remove this default set of views, although they can be added back with a script if needed:
...
A view key will be generated for you if not specified; **automatically generated view keys are not guaranteed to be stable over time, and you will likely lose manual layout information**.
...
With a dynamic view, you’re showing *instances* of relationships that are defined in the static model. For example, imagine that you have two software systems defined in the static model, with a single relationship between them described as “Sends data to”. A dynamic view allows you to override the relationship description, to better describe the interaction in the context of the behaviour you’re diagramming.

See [DSL cookbook - Dynamic view](https://docs.structurizr.com/dsl/cookbook/dynamic-view/) and [DSL cookbook - Dynamic view with parallel sequences](https://docs.structurizr.com/dsl/cookbook/dynamic-view-parallel/) for examples.
...
The `custom` keyword is used to define a custom view. Only custom elements are permitted to be included on a custom view.
...
See [DSL cookbook - Custom view](https://docs.structurizr.com/dsl/cookbook/custom-view) for an example.
...
You will need to provide a PlantUML/Mermaid/Kroki URL, and optionally a format (`png` or `svg`), when using these services. These can be specified as view set properties:
...
See [DSL cookbook - Image view](https://docs.structurizr.com/dsl/cookbook/image-view/) and
...
Elements can either be specified using individual identifiers, the wildcard identifier (`*`) or an [element expression](https://docs.structurizr.com/dsl/expressions#element-expressions). Please note that including elements will also include the relationships between those elements.
...
To exclude a relationship in a view, you can specify an individual relationship identifier or a [relationship expression](https://docs.structurizr.com/dsl/expressions#relationship-expressions):

```
exclude <identifier|expression> [identifier|expression...]
```

Alternatively, you can use the following syntax (please note the double quotes surrounding the entire expression) to target one or more relationships:
...
Please note that if your DSL workspace does not explicitly define any views, the DSL parser will automatically create a default set of views for you, with auto-layout enabled. To change this behaviour, you can either (1) explicitly define your views or (2) use a script to disable automatic layout ([example](https://docs.structurizr.com/dsl/cookbook/scripts/#create-the-default-views-without-automatic-layout)).
...
The `element` keyword is used to define an element style. All nested properties (`shape`, `icon`, etc) are optional.
...
- [DSL cookbook - Element styles](https://docs.structurizr.com/dsl/cookbook/element-styles/)
- [DSL cookbook - Groups](https://docs.structurizr.com/dsl/cookbook/groups/)
- [Notation](https://docs.structurizr.com/server/diagrams/notation)

Notes:

- Colors can be specified as a hex code (e.g. `#ffff00`) or a CSS/HTML named color (e.g. `yellow`).
- See [Notation - Icons](https://docs.structurizr.com/server/diagrams/notation#icons) for information about HTTPS/CORS if you are using the Structurizr playground, local, or server and specifying an element style icon via a URL.
- Element styles are designed to work with the Structurizr playground, local, or server, and may not be fully supported by the [PlantUML, Mermaid, etc export formats](https://docs.structurizr.com/export) (e.g. shapes and icons).

## relationship style

The `relationship` keyword is used to define a relationship style. All nested properties (`thickness`, `color`, etc) are optional.
...
- [DSL cookbook - Relationship styles](https://docs.structurizr.com/dsl/cookbook/relationship-styles/)
- [Notation](https://docs.structurizr.com/server/diagrams/notation)

Notes:

- Colors can be specified as a hex code (e.g. `#ffff00`) or a CSS/HTML named color (e.g. `yellow`).
- Relationship styles are designed to work with the Structurizr playground, local, or server, and may not be fully supported by the [PlantUML, Mermaid, etc export formats](https://docs.structurizr.com/export) (e.g. line/arrow colours).

## theme

The `theme` keyword can be used to specify a theme that should be used when rendering diagrams. See [Themes](https://docs.structurizr.com/server/diagrams/themes) for more details.

```
theme <name|url|file>
```
- Themes specified by name need to be [installed](https://docs.structurizr.com/server/diagrams/themes).
...
The `themes` keyword can be used to specify one or more themes that should be used when rendering diagrams. See [Themes](https://docs.structurizr.com/server/diagrams/themes) for more details.
...
The `scope` keyword can be used to set the [workspace scope](https://docs.structurizr.com/workspaces).
...
See [Includes](https://docs.structurizr.com/dsl/includes) for more details.
...
See [Identifiers](https://docs.structurizr.com/dsl/identifiers) for more details.
...
See [Implied relationships](https://docs.structurizr.com/dsl/implied-relationships) for more details.
...
See [Documentation](https://docs.structurizr.com/dsl/docs) for more details.
...
See [Architecture decision records](https://docs.structurizr.com/dsl/adrs) for more details.

## !components

The `!components` keyword is a DSL wrapper around the [Structurizr for Java component finder](https://docs.structurizr.com/java/component), providing the ability to automatically discover components in a Java codebase.
...
See [Scripts](https://docs.structurizr.com/dsl/scripts) for more details.
...
See [Plugins](https://docs.structurizr.com/dsl/plugins) for more details.